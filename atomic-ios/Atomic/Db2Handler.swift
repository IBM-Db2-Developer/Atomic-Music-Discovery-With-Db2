...
public class Db2Handler {
    public struct AuthSettings: Codable {
        var hostnameREST: String
        var hostnameDB: String
        var database: String
        var dbPort: Int
        var restPort: Int
        var ssl: Bool
        ...
        public init(hostnameREST: String, hostnameDB: String, database: String, dbPort: Int, restPort: Int, ssl: Bool, password: String, username: String, expiryTime: String) {
            self.hostnameREST = hostnameREST
            self.hostnameDB = hostnameDB
    ...
    public actor Job<T: Codable> {
        ...
        init(jobID: String, limit: Int, authSettings: AuthSettings, authToken: String) throws {
            guard let nextPageURL = URL(string: "http://\(authSettings.hostnameREST):\(authSettings.restPort)/v1/services/\(jobID)") else {
            ...
            guard let stopURL = URL(string: "http://\(authSettings.hostnameREST):\(authSettings.restPort)/v1/services/stop/\(jobID)") else {
                throw RequestError.invalidURL
            }
    ...
    private func getDb2AuthToken() async throws -> String {
        guard let authUrl = URL(string: "http://\(authSettings.hostnameREST):\(authSettings.restPort)/v1/auth") else {
            throw RequestError.invalidURL
        }

        let body: [String: Any] = [
            "dbParms": [
                "dbHost": authSettings.hostnameDB,
    ...
    private func runQuery(service: String, version: String, parameters: [String: Any], sync: Bool) async throws -> Data {
        guard let url = URL(string: "http://\(authSettings.hostnameREST):\(authSettings.restPort)/v1/services/\(service)/\(version)") else {
            throw RequestError.invalidURL
        }
    ...
    private func runSQL(statement: String, parameters: [String: Any], isQuery: Bool, sync: Bool) async throws -> Data {
        guard let url = URL(string: "http://\(authSettings.hostnameREST):\(authSettings.restPort)/v1/services/execsql") else {
            throw RequestError.invalidURL
        }
